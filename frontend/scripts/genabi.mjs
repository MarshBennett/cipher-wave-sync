...
const CONTRACT_NAMES = ["FHECounter", "EncryptedMessages"];

// Hardhat project root (this repo)
// In this setup, the frontend lives in `<root>/frontend` and the contracts in `<root>`
// so we just go one level up.
const rel = "..";
...
if (!fs.existsSync(dir)) {
  console.error(
    `${line}Unable to locate Hardhat project root at '${dir}'.\n` +
      `Please run this command from within the frontend folder of the cipher-wave-sync repo.${line}`
  );
  process.exit(1);
}
...
for (const CONTRACT_NAME of CONTRACT_NAMES) {
  ...
  if (deployLocalhost && deploySepolia) {
    if (
      ...
      console.warn(
        `${line}WARNING: Deployments on localhost and Sepolia differ for ${CONTRACT_NAME}. Using localhost ABI. Consider re-deploying the contracts on Sepolia if needed.${line}`
      );
      // Use localhost ABI for both (localhost has the latest contract with mock functions)
